...
const buildSummary = (profile: VisitorProfile) => {
  ...
  const parts = [primary.address, primary.country, primary.region, primary.isp].filter(Boolean)
...
export const buildVisitorProfile = async (): Promise<VisitorProfile> => {
  ...
  const [ipv4, ipv6] = await Promise.all([fetchIpIntel(ipv4Base), fetchIpIntel(ipv6Base)])
